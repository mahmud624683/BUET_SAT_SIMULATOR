...
def custom_deepcopy(obj, memo=None):
    ...
    # Check if the object is a ctypes object
    if isinstance(obj, (ctypes.Array, ctypes.Structure, ctypes.Union, ctypes._Pointer)):
        return obj
    ...
    if isinstance(obj, list):
        ...
        result = []
        memo[id(obj)] = result
        result.extend(custom_deepcopy(item, memo) for item in obj)
        return result
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            result[custom_deepcopy(key, memo)] = custom_deepcopy(value, memo)
        return result
    ...
    if isinstance(obj, tuple):
        if id(obj) in memo:
            return memo[id(obj)]
        result = tuple(custom_deepcopy(item, memo) for item in obj)
        memo[id(obj)] = result
        return result

    # Fallback to default deepcopy
...
def eager_dll_attack(args):
 ...
 #           cpy_dscinp = custom_deepcopy(dscinp)
