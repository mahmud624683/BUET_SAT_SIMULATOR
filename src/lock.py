...
keys=["1011"]#,"1100011010101100","001010101111000101011101","10011000111110101010111101010101"]#,"111111111001010101000000000011111101010111110101"]
...
for ckt in ckt_name:
    for algo in algo_name:
        for key in keys:
            ...
            lk_file =f"{des_folder}/{ckt}_{algo}_{bit_no}k.bench"
            if os.path.exists(lk_file):
                ...
                if algo == "rll": algo_methods.RLL(src_file,lk_file,key)
                    ...
                    src_file = f"{des_folder}/{ckt}_rll_{bit_no}k.bench"
                    libar_bit_no = math.ceil(33*int(bit_no)/100)
                    if libar_bit_no<8: libar_bit_no=2
